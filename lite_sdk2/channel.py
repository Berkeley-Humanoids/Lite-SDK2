"""Message-agnostic CycloneDDS transport: a process-wide participant plus
...
from .topics import default_qos, default_topic, ros_topic_to_dds
...
# warnings are genuinely useful out of the box. Attach a stderr handler (once)
# if the consumer hasn't configured `lite_sdk2.channel` logging yet.
...
def initialize(domain_id: int = 0, network_interface: str | None = None) -> None:
    ...
    Call once before constructing publishers/subscribers. Calling again with
    different arguments tears down and rebuilds the participant. ``domain_id``
    must match ``ROS_DOMAIN_ID`` on the bringup.
    """
...
def publisher(
    ...
    *,
    ...
    resolved_topic = topic if topic is not None else default_topic(message_type)
    resolved_qos = qos if qos is not None else default_qos(message_type)
    return ChannelPublisher(resolved_topic, message_type, resolved_qos, domain_id, network_interface)
...
def subscriber(
    ...
    *,
    ...
    resolved_topic = topic if topic is not None else default_topic(message_type)
    resolved_qos = qos if qos is not None else default_qos(message_type)
    return ChannelSubscriber(resolved_topic, message_type, resolved_qos, domain_id, network_interface)


class ChannelPublisher(Generic[MessageT]):
    """DDS publisher for a single topic.

    ``topic`` accepts a ROS-style name (``/lowcommand``, ``lowcommand``) or an
    already-prefixed DDS name (``rt/lowcommand``); both resolve to the same wire
    topic. ``topic_name`` holds the final DDS name.
    """
    ...
    def initialize(self) -> None:
        ...
        self._topic = Topic(self._factory.participant, self.topic_name, self.message_type, qos=self.qos)
        self._writer = DataWriter(self._factory.publisher, self._topic, qos=self.qos)
...
class ChannelSubscriber(Generic[MessageT]):
    """DDS subscriber for a single topic. See :class:`ChannelPublisher` for the
    ``topic`` name rules."""
    ...
    def initialize(
        ...
        self._factory = _get_factory(self.domain_id, self.network_interface)
        self._topic = Topic(self._factory.participant, self.topic_name, self.message_type, qos=self.qos)
        self._reader = DataReader(self._factory.subscriber, self._topic, qos=self.qos)
